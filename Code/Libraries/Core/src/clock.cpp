...
,	m_RealUnpausedCurrentTime( 0.0f )
,	m_RealUnpausedDeltaTime( 0.0f )
,	m_TickCount( 0 )
,	m_Resolution( 0.0 )
...
void Clock::Tick( bool GamePaused /*= false*/, const bool DoGameTick /*= true*/ )
{
	...
	if( DoGameTick )
	{
		...
		else if( m_MultiplierRequests.Size() )
		{
			...
			m_RealUnpausedDeltaTime	= m_MachineDeltaTime;
		}
		else
		{
		...
		m_RealUnpausedCurrentTime	= m_RealUnpausedCurrentTime	+ m_RealUnpausedDeltaTime;
	}
...
void Clock::GameTick( const bool GamePaused, const float DeltaTime )
{
	...
	else if( m_MultiplierRequests.Size() )
	{
		...
		m_RealUnpausedDeltaTime		= DeltaTime;
		m_GameCurrentTime			= m_GameCurrentTime			+ m_GameDeltaTime;	// NOTE: Accumulates drift
		m_RealUnpausedCurrentTime	= m_RealUnpausedCurrentTime	+ m_RealUnpausedDeltaTime;
	}
	else
...
float Clock::GetRealUnpausedCurrentTime() const
{
	return m_RealUnpausedCurrentTime;
}

float Clock::GetRealUnpausedDeltaTime() const
{
	return m_RealUnpausedDeltaTime;
}
...
void Clock::Report() const
{
	...
	DEBUGPRINTF( "Real unpaused time: %.2f\n", m_RealUnpausedCurrentTime );
	DEBUGPRINTF( "Machine delta time: %.4f\n", m_MachineDeltaTime );
	DEBUGPRINTF( "Game delta time: %.4f\n", m_GameDeltaTime );
	DEBUGPRINTF( "Real unpaused delta time: %.4f\n", m_RealUnpausedDeltaTime );
	DEBUGPRINTF( "Game time drift: %.4f\n", m_GameCurrentTime - m_MachineCurrentTime );	// Only meaningful before time effects are applied
	DEBUGPRINTF( "Real unpaused time drift: %.4f\n", m_RealUnpausedDeltaTime - m_MachineCurrentTime );
}
