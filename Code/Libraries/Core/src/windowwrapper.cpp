...
int Window::Init( const char* Title, const char* ClassName, DWORD Style, DWORD ExStyle, int Width, int Height, HINSTANCE hInstance, WNDPROC WndProc /*= NULL*/, uint Icon /*= 0*/, int ScreenWidth /*= 0*/, int ScreenHeight /*= 0*/ )
{
	...
	// HACKHACK for ultrawide/multimonitor
	STATICHASH( DisplayX );
	const int DisplayX = ConfigManager::GetInt( sDisplayX );

	STATICHASH( DisplayY );
	const int DisplayY = ConfigManager::GetInt( sDisplayY );

	// Center window, or just position in top-left (or at HACKHACK DisplayX/Y) if it won't fit
	const int DesiredWindowX	= ( ScreenWidth - AdjustedWidth ) >> 1;
	const int DesiredWindowY	= ( ScreenHeight - AdjustedHeight ) >> 1;
	const int WindowX			= ( DesiredWindowX >= 0 ) ? DesiredWindowX : DisplayX;
	const int WindowY			= ( DesiredWindowY >= 0 ) ? DesiredWindowY : DisplayY;
...
#if BUILD_WINDOWS_NO_SDL
// TODO: Add some failure handling, and meaningful return values
int Window::Show( int nCmdShow /*= SW_SHOWNORMAL*/ )
{
	ShowWindow( m_hWnd, nCmdShow );
	SetForegroundWindow( m_hWnd );	// DLP 4 May 2020: Maybe stop the window disappearing behind Steam when launched?
	SetWindowPos( m_hWnd, HWND_TOP, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE );	// Make sure we're on top (DLP 4 May 2020: this used to be HWND_NOTOPMOST, trying to fix the window disappearing behind Steam)
	UpdateWindow( m_hWnd );
	return 0;
}
...
int Window::SetPosition( int x, int y )
{
	...
	{
#if BUILD_WINDOWS_NO_SDL
		SetWindowPos( m_hWnd, HWND_TOP, x, y, 0, 0, SWP_NOSIZE );	// DLP 4 May 2020: this used to be HWND_NOTOPMOST, trying to fix the window disappearing behind Steam
#endif
#if BUILD_SDL
...
int Window::SetSize( int Width, int Height, bool AdjustForStyle )
{
	...
	const BOOL Success = SetWindowPos( m_hWnd, HWND_TOP, 0, 0, Width, Height, SWP_NOMOVE );	// DLP 4 May 2020: this used to be HWND_NOTOPMOST, trying to fix the window disappearing behind Steam
	ASSERT( Success );
	Unused( Success );
...
int Window::SetSize( int Width, int Height, int ScreenWidth, int ScreenHeight, bool AdjustForStyle )
{
	...
	const BOOL Success = SetWindowPos( m_hWnd, HWND_TOP, WindowX, WindowY, Width, Height, 0 );	// DLP 4 May 2020: this used to be HWND_NOTOPMOST, trying to fix the window disappearing behind Steam
	ASSERT( Success );
	Unused( Success );
