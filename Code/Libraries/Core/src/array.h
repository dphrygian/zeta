...
#define FOR_EACH_INDEX_REVERSE( idx, num )		for( uint idx = (uint)(num) - 1; idx != INVALID_ARRAY_INDEX; --idx )
...
public:
	...
	// Compare the contents of each array.
	...
	bool operator==( const Array& Other ) const
	{
		...
		FOR_EACH_INDEX( Index, m_Size )
		{
			if( m_Array[ Index ] == Other.m_Array[ Index ] )
			{
				continue;
			}
	...
	// Allows Arrays to be used as Map keys.
	// Requires C::operator< to be implemented.
	bool operator<( const Array& Other ) const
	{
		if( m_Size < Other.m_Size )
		...
		FOR_EACH_INDEX( Index, m_Size )
		{
			...
			if( Other.m_Array[ Index ] < m_Array[ Index ] )
			{
				return false;
			}
		}
	...
	void	Insert( const C& Item, const uint Index )
	{
		if( Index == m_Size )
		{
			PushBack( Item );
	...
	// Requires C::operator== to be implemented
	// See also Search for the iterator-based version
	...
	// See also Find
	bool Contains( const C& Item ) const
	{
		return Search( Item ).IsValid();
	}
	...
	const C&	operator[]( const uint Index ) const
	{
		DEVASSERTDESC( Index < m_Size, "Array index out of bounds." );
		return m_Array[ Index ];
	}
	...
	// This form works for lambdas/closures as well as static function pointers.
	template<typename TArraySortFunc> void QuickSort( TArraySortFunc SortFunc )
	{
		if( m_Size > 1 )
		{
			const uint PivotIndex = m_Size / 2;
	...
	// This form works for lambdas/closures as well as static function pointers.
	template<typename TArraySortFunc> void InsertionSort( TArraySortFunc SortFunc )
	{
		for( uint Index = 0; Index < m_Size; ++Index )
		{
			// Create a copy of the item. It's possible I don't want to do this in some cases.
