...
void InnerParse( const char c, char& StrMark, Array<char>& TokenString, const int LineCount, SToken::ETokenType& InOutTokenType, bool* OutClosedString = NULL )
{
	...
	case SToken::ET_None:
		...
		{
			// 24 Nov 2019: New behavior, a leading underscore in a value means unset (or ignore) the key.
			// This is mostly used to make some Loom script easier, so I can use defaults in color or vector macros.
			InOutTokenType = SToken::ET_Unset;
		}
		else
		{
	...
	case SToken::ET_Unset:
		// Do nothing with the rest of the value, but warn because there shouldn't be anything beyond the underscore
		PRINTF( "Unexpected character %c in Unset token at line %d\n", c, LineCount );
		WARNDESC( "Unexpected character in Unset token" );
		break;
...
void ConfigParser::Parse( const IDataStream& Stream, const bool WarnIfOverwriting /*= false*/ )
{
		...
		else if( c == '\n' || c == '\v' )
		{
			...
			if( Token.m_TokenString.Empty() )
			{
				if( Token.m_TokenType != SToken::ET_Counter &&
					Token.m_TokenType != SToken::ET_Unset )
				{
					Token.m_TokenType = SToken::ET_None;
				}
				Tokens.PushBack( Token );
	...
	for( uint i = 0; i < Tokens.Size(); ++i )
	{
		...
		if( NameToken.m_TokenType == SToken::ET_Name )
		{
			...
			case SToken::ET_Unset:
				{
					// Don't just ignore, actually unset the key
					CONDITIONAL_OVERWRITE_ASSERT;
					ConfigManager::Unset( Name, Context );
				}
				break;
			...
			default:
				PRINTF( "Unexpected token %s\n", SToken::m_TokenNames[ ValueToken.m_TokenType ] );
				WARNDESC( "Unexpected token" );
				break;
			}
