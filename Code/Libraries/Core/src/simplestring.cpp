...
SimpleString SimpleString::Replace( const char* const Find, const char* const Replace ) const
{
	...
	{
		const char* const		Start	= NewStringBuffer.GetData() + Iterator;
		const char* const		SubStr	= strstr( Start, Find );
		if( SubStr )
		{
...
void SimpleString::SplitInPlace( uint Index, SimpleString& Left, SimpleString& Right ) const
{
	if( Index < m_Length )
	{
...
void SimpleString::SplitInPlaceFind( const char Delimiter, SimpleString& Left, SimpleString& Right ) const
{
	char Find[2];
	Find[0] = Delimiter;
	Find[1] = '\0';
...
#define PERCENT_ENCODE_CHAR														\
	{																			\
		const char			NextChar		= EncodedArray[ CharIter ];			\
		const c_uint8		UnsignedChar	= static_cast<c_uint8>( NextChar );	\
