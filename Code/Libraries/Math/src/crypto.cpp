...
// DLP 22 May 2021: Moved into a namespace to prevent global i colliding with local i in array.h
namespace ARC4
{
	char		S[256];
	uint		i, j;
...
{
	ARC4::ARC4Initialize( Key );

	c_uint8 Nonce = (char)Math::Random( 256 );
	for( uint Step = 0; Step < Nonce + ARC4::FixedStep; ++Step )
	{
		ARC4::ARC4Step();
	}

	OutCiphertext.Clear();
	OutCiphertext.Resize( Plaintext.Size() + ARC4::Overhead );

	for( uint Index = 0; Index < Plaintext.Size(); ++Index )
	{
		OutCiphertext[ Index ] = Plaintext[ Index ] ^ ARC4::ARC4Step();
	}
...
void Crypto::Decrypt( const Array< char >& Ciphertext, const Array< char >& Key, Array< char >& OutPlaintext )
{
	...
	OutPlaintext.Clear();
	OutPlaintext.Resize( Ciphertext.Size() - ARC4::Overhead );

	for( uint Index = 0; Index < Ciphertext.Size() - ARC4::Overhead; ++Index )
	{
		OutPlaintext[ Index ] = Ciphertext[ Index ] ^ ARC4::ARC4Step();
	}
}
