...
UIScreen* UIFactory::CreateScreen( const SimpleString& DefinitionName )
{
	...
	HashedString UIScreenType = ConfigManager::GetInheritedHash( sUIScreenType, sBase, sDefinitionName );
...
UIWidget* UIFactory::CreateWidget( const SimpleString& DefinitionName, UIScreen* const pOwnerScreen, UIWidget* const pParentWidget )
{
...
#if BUILD_FINAL
	STATICHASH( DevOnly );
	const bool DevOnly = ConfigManager::GetInheritedBool( sDevOnly, false, sDefinitionName );
	if( DevOnly )
	{
		return NULL;
	}
	...
	HashedString UIWidgetType = ConfigManager::GetInheritedHash( sUIWidgetType, "", sDefinitionName );
