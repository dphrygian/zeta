...
#if BUILD_DEBUG
#define BEGIN_ITERATING_STACK do{ ++m_IteratingStackRefCount; } while(0)
#define END_ITERATING_STACK do{ --m_IteratingStackRefCount; } while(0)
#define CHECK_ITERATING_STACK do{ DEVASSERT( 0 == m_IteratingStackRefCount ); } while(0)
#else
...
bool UIStack::SeizesMouse()
{
	...
	FOR_EACH_ARRAY( ScreenIter, m_ScreenStack, SStackScreen )
	{
		...
		if( StackScreen.m_Screen->SeizesMouse() )
		{
			return true;
		}
	}
...
void UIStack::Tick( float MachineDeltaTime, float GameDeltaTime, bool Render )
{
	...
	FOR_EACH_ARRAY( ScreenIter, m_ScreenStack, SStackScreen )
	{
		...
		if( TickReturn == UIScreen::ETR_Close && pScreen->CanClose() )
		{
			...
			SStackScreen& ReStackScreen = ScreenIter.GetValue();
			ReStackScreen.m_Popped = true;
		}
	}
	END_ITERATING_STACK;
