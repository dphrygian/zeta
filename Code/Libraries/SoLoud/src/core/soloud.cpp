...
namespace SoLoud
{
	...
	Soloud::Soloud()
	{
		...
		// DLP 27 Nov 2021...: Early mutex to prevent game thread blocking while SoLoud is mixing.
		mMixerMutex = NULL;
		// ...DLP 27 Nov 2021
		mPostClipScaler = 0;
	...
	void Soloud::deinit()
	{
		...
		// DLP 27 Nov 2021...: Early mutex to prevent game thread blocking while SoLoud is mixing.
		if (mMixerMutex)
			Thread::destroyMutex(mMixerMutex);
		mMixerMutex = NULL;
		// ...DLP 27 Nov 2021
	}
	...
	result Soloud::init(unsigned int aFlags, unsigned int aBackend, unsigned int aSamplerate, unsigned int aBufferSize, unsigned int aChannels)
	{
		...
		// DLP 27 Nov 2021...: Early mutex to prevent game thread blocking while SoLoud is mixing.
		mMixerMutex = Thread::createMutex();
		// ...DLP 27 Nov 2021
...
#define FIXPOINT_FRAC_MASK ((1 << FIXPOINT_FRAC_BITS) - 1)
	...
	// DLP 27 Nov 2021...: Early mutex to prevent game thread blocking while SoLoud is mixing.
	bool Soloud::tryLockMixerMutex()
	{
		if (mMixerMutex)
			return Thread::tryLockMutex(mMixerMutex);
		return true;
	}

	void Soloud::lockMixerMutex()
	{
		if (mMixerMutex)
			Thread::lockMutex(mMixerMutex);
	}

	void Soloud::unlockMixerMutex()
	{
		if (mMixerMutex)
			Thread::unlockMutex(mMixerMutex);
	}
	// ...DLP 27 Nov 2021
